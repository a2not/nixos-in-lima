{
  ...
  script_mounts =
    if LIMA_CIDATA_MOUNTTYPE != "9p"
    then
      (lib.concatStringsSep "\n" (lib.forEach LIMA_MOUNTPOINTS (
        mountpoint: ''
          mkdir -p "${mountpoint}";
  ...
  fileSystemsMount = lib.zipAttrsWith (name: values: (lib.elemAt values 0)) (lib.forEach (userData."mounts") (
    row: {
      ${(lib.elemAt row 1)} = {
...
in {
  ...
  programs.fuse =
    if LIMA_CIDATA_MOUNTTYPE == "reverse-sshfs"
    then {userAllowOther = true;}
    else {};
